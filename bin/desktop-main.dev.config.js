...
if(process.env.REACT_BUILD_DEV_CACHE == 'enabled') {
  ...
  webpackPlugins.push(
    new HardSourceWebpackPlugin({
      cacheDirectory: path.join('../../build-cache', 'desktop-main-dev', '[confighash]'),
      recordsPath: path.join('../../build-cache', 'desktop-main-dev', '[confighash]', 'records.json'),
      
      configHash: function(webpackConfig) {
        return `webpack-desktop-main-dev`;
      },
...
const options = {
  ...
  entry: [
    'babel-polyfill',
    path.join(__dirname, '../src/desktop/electron-entry'),
  ],
  output: {
